...
{
	if(COIN_DETECT1 == IR_DETECT_ON)    // 红外对射传感器
		dgus_tf1word(ADDR_DETCET1,1);	//被遮挡
	else
		dgus_tf1word(ADDR_DETCET1,0);	//未被遮挡
	if(COIN_DETECT2 == IR_DETECT_ON)    // 红外对射传感器
		dgus_tf1word(ADDR_DETCET2,1);	//被遮挡
	else
		dgus_tf1word(ADDR_DETCET2,0);	//未被遮挡
}
